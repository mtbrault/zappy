...
bool MyEventReceiver::OnEvent(const irr::SEvent& event)
{
	...
	if (event.EventType == irr::EET_MOUSE_INPUT_EVENT) {
		if (event.MouseInput.Event == irr::EMIE_LMOUSE_PRESSED_DOWN) {
			...
			_mouseClickLastPos = std::make_pair(MouseState.Position.X,
...
std::pair<int, int>	MyEventReceiver::getLastCLickPos()
{
	return (_mouseClickLastPos);
}

bool	MyEventReceiver::isClick()
{
	if (_click) {
		_click = false;
		return (true);
