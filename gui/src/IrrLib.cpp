...
IrrLib::IrrLib(Actions &KeyIsDown)
	:_actions(KeyIsDown), _ground(nullptr), _screenSizeX(SCREEN_WIDTH), _screenSizeY(SCREEN_HEIGHT), _splitScreen(false), _lastFps(0), _altitude(10), _x(5), _y(5)
{
	...
	_colsmgr = _smgr->getSceneCollisionManager();
	...
	_factoryDelete.insert(std::make_pair(Entity::SPHERE, std::bind(&IrrLib::removeSphere, this,
		std::placeholders::_1)));
	_factoryDelete.insert(std::make_pair(Entity::PLAYER, std::bind(&IrrLib::removePlayer, this,
		std::placeholders::_1)));
	_camTarget = irr::core::vector3df(0, 0, 0);
	_camera->setPosition(irr::core::vector3df(0, 10, 0));
	_camPos = irr::core::vector3df(0, 10, 0);
	_camera->setInputReceiverEnabled(true);
...
void	IrrLib::createPlane(pairUC &size)
{
	...
	_ground->setMaterialTexture(0, _driver->getTexture("./media/square_tile.bmp"));
...
Actions	IrrLib::getActions()
{
	...
	if (_eventReceiver.IsKeyDown(irr::KEY_KEY_W))
		_actions.W = true;
	if (_eventReceiver.isClick()) {
		_actions.click = true;
		_actions.lastMouseClick = _eventReceiver.getLastCLickPos();
	}
	_camera->setPosition(irr::core::vector3df(_camPos.X, _altitude, _camPos.Z));
	_camera->setTarget(irr::core::vector3df(_camPos.X, 0, _camPos.Z));
	return (_actions);
}

int		IrrLib::handleClickColision()
{
	irr::core::line3d<irr::f32> ray = _colsmgr->getRayFromScreenCoordinates(_device->getCursorControl()->getPosition(), _camera);
	return (_colsmgr-> getSceneNodeFromRayBB(ray)->getID());
}
...
void IrrLib::updateButton(std::unique_ptr<IEntity> &entity)
{
	for (auto &it : _buttons) {
		if (static_cast<MenuItem*>(entity.get())->getId() == it->getID()) {
			if (static_cast<MenuItem*>(entity.get())->isSetTexture())
				it->setImage(_driver->getTexture(static_cast<MenuItem*>(entity.get())
					->getTexture().c_str()));
			return;
		}
	}
...
void IrrLib::addEditBox(std::unique_ptr<IEntity> &entity)
{
	...
	editbox->setPasswordBox(item->isPasswordBox());
...
std::string IrrLib::getInputText(std::unique_ptr<IEntity> &item)
{
	for (auto it = _inputs.begin(); it != _inputs.end(); it++) {
		if ((*it)->getID() == static_cast<MenuItem*>(item.get())->getId()) {
			std::wstring tmp = (*it)->getText();
			std::string toto(tmp.begin(), tmp.end());
			return (toto);
		}
	}
	return ("");
...
void IrrLib::draw()
{
	...
	// _driver->setViewPort(irr::core::rect<irr::s32>(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT));
...
void IrrLib::drawGame()
{
	...
	// if (_splitScreen == false) {
		...
		// irr::core::vector3df camPos = _players[0]->getPosition();
		_camera->setPosition(irr::core::vector3df(0, _altitude, 0));
...
void IrrLib::updatePlayer(std::unique_ptr<IEntity> &entity)
{
	for (auto &it : _players) {
		if (it->getID() == static_cast<Player*>(entity.get())->getId()) {
			...
			it->setPosition(irr::core::vector3df(entity->getPos().first, 1, entity->getPos().second));
...
{
	irr::scene::IAnimatedMesh* mesh = _smgr->getMesh(static_cast<Player*>(entity.get())->getModel().c_str());
	irr::scene::IAnimatedMeshSceneNode* node = _smgr->addAnimatedMeshSceneNode(mesh);
	if (node) {
		node->setMaterialFlag(irr::video::EMF_LIGHTING, false);
		node->setMD2Animation(irr::scene::EMAT_STAND);
		node->setMaterialTexture( 0, _driver->getTexture(static_cast<Player*>(entity.get())->getTexture().c_str()));
		node->setScale(irr::core::vector3df(static_cast<Player*>(entity.get())->getScale(),
			static_cast<Player*>(entity.get())->getScale(),
			static_cast<Player*>(entity.get())->getScale()));
		node->setPosition(irr::core::vector3df(entity->getPos().first, 1, entity->getPos().second));
		node->setID(static_cast<Player*>(entity.get())->getId());
		node->setRotation(irr::core::vector3df(0, static_cast<Player*>(entity.get())->getRotation(), 0));
		_players.push_back(node);
	}
}

void IrrLib::removePlayer(int id)
{
	int i = 0;
...
void IrrLib::updateEntities(std::vector<std::unique_ptr<IEntity>> &gameEntities)
{
	for (auto &it : gameEntities) {
